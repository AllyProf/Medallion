...
class AccountantController extends Controller
{
    ...
    public function waiterTrends(Request $request)
    {
        ...
        // 1. DYNAMIC PERIOD & FILTER HANDLING
        $period = $request->get('period', 'today');
        $startDateInput = $request->get('start_date');
        $endDateInput = $request->get('end_date');
        $waiterIdFilter = $request->get('waiter_id');
        ...
        if ($startDateInput) {
            ...
            if ($period == '30days') $startDate = now()->subDays(29)->startOfDay();
            elseif ($period == 'month') $startDate = now()->startOfMonth()->startOfDay();
            elseif ($period == '7days') $startDate = now()->subDays(6)->startOfDay();
            else {
                $startDate = now()->startOfDay(); // Default: Today
                $period = 'today';
            }
        }

        // 2. FETCH ALL WAITERS (For Filter Dropdown)
        $allWaiters = Staff::where('user_id', $ownerId)
            ->whereHas('role', function($q) {
                $q->where('name', 'Waiter');
            })->orderBy('full_name')->get();

        // 3. BUILD DATE LABELS & FLOW
        $labels = [];
        $dateRange = [];
        for ($d = $startDate->copy(); $d <= $endDate; $d->addDay()) {
            $labels[]    = $d->format('M d');
        ...
        if ($waiterIdFilter) {
            $ordersQuery->where('waiter_id', $waiterIdFilter);
        }
        
        $orders = $ordersQuery->with(['waiter', 'items', 'kitchenOrderItems'])->get();

        // 5. DAILY AGGREGATION (Overall Performance)
        $dailyBreakdown = collect();
        foreach ($dateRange as $dayStr) {
            $dayOrders = $orders->filter(fn($o) => Carbon::parse($o->created_at)->format('Y-m-d') === $dayStr);
            
            $dayRevenue = $dayOrders->sum(function($order) {
                $bar = $order->items ? $order->items->where('status', '!=', 'cancelled')->sum('total_price') : 0;
                $food = $order->kitchenOrderItems ? $order->kitchenOrderItems->where('status', '!=', 'cancelled')->sum('total_price') : 0;
                return $bar + $food;
            });

            $topWaiterId = $dayOrders->groupBy('waiter_id')
                ->map(fn($o) => $o->sum(function($order) {
                    return ($order->items ? $order->items->where('status', '!=', 'cancelled')->sum('total_price') : 0) + 
                           ($order->kitchenOrderItems ? $order->kitchenOrderItems->where('status', '!=', 'cancelled')->sum('total_price') : 0);
                }))
                ->sortDesc()
                ->keys()
                ->first();
            
            $topWaiterName = $topWaiterId ? ($allWaiters->firstWhere('id', $topWaiterId)->full_name ?? 'N/A') : '--';

            $dailyBreakdown->push([
                'date' => $dayStr,
                'label' => Carbon::parse($dayStr)->format('M d, Y'),
                'revenue' => $dayRevenue,
                'orders_count' => $dayOrders->count(),
                'top_waiter' => strtoupper(explode(' ', trim($topWaiterName))[0])
            ]);
        }

        // 6. WAITER LEADERBOARD DATA
        ...
        foreach ($grouped as $waiterId => $waiterOrders) {
            ...
            $dailyFlow = array_map(function($dayStr) use ($waiterOrders) {
                $dayOrders = $waiterOrders->filter(fn($o) => Carbon::parse($o->created_at)->format('Y-m-d') === $dayStr);
        ...
        // 7. PASS DATA TO VIEW
        return view('accountant.reports.waiter-trends', [
            'waiterData' => $waiterData,
            'labels' => $labels,
            'totalWaitersRevenue' => $totalWaitersRevenue,
            'period' => $period,
            'startDate' => $startDate->format('Y-m-d'),
            'endDate' => $endDate->format('Y-m-d'),
            'waiterId' => $waiterIdFilter,
            'allWaiters' => $allWaiters,
            'dailyBreakdown' => $dailyBreakdown->reverse()->values()
        ]);
    }
}
