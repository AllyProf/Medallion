...
<div class="row mb-4">
    ...
    <div class="widget-small primary coloured-icon shadow-sm" style="border-radius: 12px; height: 100%;">
      ...
      <div class="info">
        ...
        <span class="badge badge-light border text-dark py-1" style="font-size: 0.65rem;">
          <i class="fa fa-shopping-cart"></i> Sales: TSh {{ number_format($shiftRevenue) }}
        </span>
      </div>
    </div>
  </div>

  {{-- 2. Shift COGS (Instead of total Daily Circulation) --}}
  <div class="col-md-3">
    <div class="widget-small info coloured-icon shadow-sm" style="border-radius: 12px; height: 100%;">
      ...
      <div class="info">
        <h4 class="text-uppercase text-muted" style="font-size: 0.65rem; letter-spacing: 1px;">Shift COGS</h4>
        <p><b class="h5 text-info">TSh {{ number_format($shiftCOGS) }}</b></p>
        <span class="text-muted" style="font-size: 0.65rem;">Cost of Goods Sold</span>
      </div>
    </div>
  </div>

  {{-- 3. Shift Profit (Gross profit generated by the shift) --}}
  <div class="col-md-3">
    <div class="widget-small success coloured-icon shadow-sm" style="border-radius: 12px; height: 100%;">
      ...
      <div class="info">
        <h4 class="text-uppercase text-muted" style="font-size: 0.65rem; letter-spacing: 1px;">Shift Profit</h4>
        <p><b class="h5 text-success">TSh {{ number_format($shiftProfit) }}</b></p>
  ...
  {{-- 4. Vault Balance (Global Daily Position) --}}
  <div class="col-md-3">
    <div class="widget-small warning coloured-icon shadow-sm" style="border-radius: 12px; height: 100%;">
      ...
      <div class="info">
        ...
        <span class="text-muted small" style="font-size: 0.65rem;">
           @if($ledger->opening_cash > 0)
              Opening: TSh {{ number_format($ledger->opening_cash) }}
           @else
...
@if($isManagementRole && !$todayHandover)
  <div class="row">
    <div class="col-md-12">
      <div class="tile text-center {{ $bar_shift ? 'border-primary' : 'border-warning' }} py-5 mb-4 shadow-sm" style="border-radius: 12px; border-top: 5px solid {{ $bar_shift ? '#009688' : '#ffc107' }};">
        <div class="mb-4">
            @if($bar_shift)
               <i class="fa fa-refresh fa-spin fa-4x text-primary opacity-50"></i>
            @else
               <i class="fa fa-coffee fa-4x text-warning opacity-50"></i>
            @endif
        </div>
        ...
        @else
            ...
            <p class="text-muted lead px-md-5 mx-auto" style="max-width: 700px;">
                All previous shifts for <strong>{{ \Carbon\Carbon::parse($date)->format('d M, Y') }}</strong> have been verified. 
                <br>The system is now waiting for the counter staff to open a new shift and begin trading.
            </p>
        @endif

        <div class="mt-4">
            <button class="btn {{ $bar_shift ? 'btn-primary' : 'btn-warning' }} btn-lg px-5 shadow-sm font-weight-bold" onclick="location.reload();">
              <i class="fa fa-refresh mr-2"></i> Refresh Sequence
            </button>
        </div>
      </div>
    </div>
